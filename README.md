...
*NOTE* Due to a bug in LLVM, most samples will fail to build for the simulator unless run through ProGuard first. The sample projects show how to use set that up, but your builds will take _much_ longer. Testing on an actual device is recommended until the bug is fixed. See [RoboVM issue #313](https://github.com/robovm/robovm/issues/313) for updates.
